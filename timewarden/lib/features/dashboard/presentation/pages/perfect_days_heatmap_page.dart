...
class _PerfectDaysHeatmapPageState extends State<PerfectDaysHeatmapPage> {
  ...
  Future<void> _loadPerfectDays() async {
    ...
    try {
      ...
      // Get perfect day dates directly from user stats
      final statsRepository = UserStatsRepositoryImpl(FirebaseService());
      final userStats = await statsRepository.getUserStats(userId);

      LogService.debug('🔍 Perfect Days Heatmap Debug:');
      LogService.debug(
          '   User Stats: ${userStats != null ? "Found" : "NULL"}');
      LogService.debug('   Perfect Days Count: ${userStats?.perfectDays ?? 0}');
      LogService.debug(
          '   Perfect Day Dates: ${userStats?.perfectDayDates.length ?? 0}');

      // Use the tracked perfect day dates from user stats
      final perfectDaysList = userStats?.perfectDayDates ?? [];

      LogService.debug(
          '✅ Loaded ${perfectDaysList.length} perfect days from user stats');
      ...
      LogService.debug('Error loading perfect days: $e');
      setState(() {
        _perfectDays = [];
        _isLoading = false;
      });
