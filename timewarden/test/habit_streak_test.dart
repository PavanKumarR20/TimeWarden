...
import 'package:timewarden/features/habits/services/habit_streak_service.dart';
...
void main() {
  group('HabitStreakService Tests', () {
    ...
    test('calculateCurrentStreak for weekly habit (3 times per week)', () {
      ...
      final lastWeekStart = currentWeekStart.subtract(const Duration(days: 7)); // Last Monday
      
      final habit = Habit(
        id: '2',
        name: 'Gym Workout',
        category: HabitCategory.health,
        frequency: const HabitFrequency(type: HabitFrequencyType.timesPerWeek, target: 3),
      ...
      final streak = HabitStreakService.calculateCurrentStreak(habit);
      expect(streak, greaterThanOrEqualTo(1)); // Should have at least 1 week streak
    });
    ...
    test('getStreakStatusMessage returns correct messages', () {
      ...
      expect(HabitStreakService.getStreakStatusMessage(dailyHabit), contains('Start your streak'));
      expect(HabitStreakService.getStreakStatusMessage(weeklyHabit), contains('streak'));
    });
  });
}
